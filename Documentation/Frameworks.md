...
If you find any difficulties using this library with your framework of choice, please [open an issue on the GitHub repository](https://github.com/jakubiszon/ZuraTDD/issues)

## `ZuraTest` attribute limitations

There are some limitations regarding the `ZuraTest` attribute. As the attribute is used to generate test methods - it needs to detect the test framework used and generate a correctly defined test. At the moment using this attribute will only work with the following frameworks:

- **MSTest**
- **XUnit**

More frameworks are planned to be added in the future.
...
- Manually creates test-case objects implementing `ITestCase<T>`.

You will need to write the methods consuming the test-case objects. Here are some examples:
- [Passing a number of TestCase objects to a method consuming `DynamicData` in **MSTest**](https://github.com/jakubiszon/ZuraTDD/blob/main/ExampleProject.Tests/ContentPublishControllerTests.cs)
